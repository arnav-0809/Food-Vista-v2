...
function cart(itemId,item,price,count){ 
    ...
    if(items.length===0)
    {
            ...
            localStorage.setItem('MOMOSITEM',JSON.stringify([...items,{itemId,item,count,price}]));
    }
    ...
    for(let i=0;i<items.length;i++)
    {
        if(items[i].item===item)
        {
            ...
            setTotalPrice(prev=>{
                if(count>counter)
                {
                    localStorage.setItem('MOMOSPRICE',JSON.stringify(parseInt(prev)+parseInt(price)));
        ...
        localStorage.setItem('MOMOSITEM',JSON.stringify([...items]));
...
const item2=()=>{databaseOrder && databaseOrder.map((i)=>{
    ...
    i.item.map((j)=>{
        // console.log(j);
         setItems(prev=>{
            localStorage.setItem('MOMOSITEM',JSON.stringify([...prev,j])); 
            return [...prev,j]
        });
      });
    setTotalPrice(prev=>{
            localStorage.setItem('MOMOSPRICE',JSON.stringify(prev+i.price));
            return prev+i.price;
        })
   } })
};
...
<Container>
    <Row className="justify-content-center">
        <Col xs={11} sm={9} md={11} lg={8} className="menuHead">MomosMenu</Col>
    </Row>
{/*1st row*/}
    <Row className="justify-content-center">
