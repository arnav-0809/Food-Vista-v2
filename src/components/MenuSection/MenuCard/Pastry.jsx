...
function cart(itemId,item,price,count){ 
    ...
    if(items.length===0)
    {
            ...
            localStorage.setItem('PASTRYITEM',JSON.stringify([...items,{itemId,item,count,price}]));
    }
    ...
    for(let i=0;i<items.length;i++)
    {
        if(items[i].item===item)
        {
            ...
            setTotalPrice(prev=>{
                if(count>counter)
                {
                    localStorage.setItem('PASTRYPRICE',JSON.stringify(parseInt(prev)+parseInt(price)));
        ...
        localStorage.setItem('PASTRYITEM',JSON.stringify([...items]));
...
const item2=()=>{databaseOrder && databaseOrder.map((i)=>{
    ...
    i.item.map((j)=>{
        // console.log(j);
         setItems(prev=>{
            localStorage.setItem('PASTRYITEM',JSON.stringify([...prev,j])); 
            return [...prev,j]
        });
      });
    setTotalPrice(prev=>{
            localStorage.setItem('PASTRYPRICE',JSON.stringify(prev+i.price));
            return prev+i.price;
        })
   } })
};
